...
class BILSTM:
  def train(self, X_train, zc_features_train, ssc_features_train, features_1d_train, Y_train, num_activation=32):
    ...
    oned_branch_input = Input(shape=(features_1d_train.shape[1],))

    # branch setup
    x_branch = Bidirectional(LSTM(num_activation))(x_branch_input)
    x_branch = Dropout(0.5)(x_branch)
    x_branch = Dense(num_activation, activation='tanh')(x_branch)
    ...
    oned_branch = Dense(num_activation, activation='linear')(oned_branch_input)
    ...
    combined = Dense(num_activation, activation='linear')(combined)
    # combined = Dropout(0.3)(combined)
    combined = Dense(Y_train.shape[1], activation='linear')(combined)
    ...
    # fit model
    self.model.fit([X_train, zc_features_train, ssc_features_train, features_1d_train], Y_train, epochs=12,
                   batch_size=64)
    print(self.model.summary())

  def evaluate(self, X_test, zc_features_test, ssc_features_test, features_1d_test, Y_test):
    self.load_model()
    test_pred = self.model.predict([X_test, zc_features_test, ssc_features_test, features_1d_test])
    true_pred = Y_test
  ...
  def predict(self, X, zc_features, ssc_features, features_1d):
    ...
    return self.model.predict([X, zc_features, ssc_features, features_1d])
